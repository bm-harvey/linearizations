...
impl Curve {
    ...
    fn from_parquet_file(file: &str) -> Result<Self> {
    ...
    fn extend_from_single_reference_curve(
        ...
        //let new_y = new_x * result.0 + result.1 + reference_curve.evaluate(new_x);
        //let new_y = reference_curve.evaluate(new_x) * result.0 + result.1;
...
fn create_extended_curves(curves: Vec<Curve>, dx: f64, x_range: Option<(f64, f64)>) -> Vec<Curve> {
    ...
    for curve in org_curves.iter_mut() {
        ...
        while ref_x < target_x {
            ...
            match (upper_spanning_curve, lower_spanning_curve) {
                ...
                (None, None) => curve.add_point(new_x, curve.evaluate(new_x)),
...
fn main() -> Result<()> {
    let curves = fs::read_dir("smoothed_lines/")
        .unwrap()
        .map(|file| {
            let path = file.unwrap().path();
            let file_name = path.to_str().unwrap();
            Curve::from_parquet_file(file_name).unwrap()
        })
        .collect::<Vec<Curve>>();

    let mut file = File::open("data/pid_det_tagged60.parquet")?;
    let df = ParquetReader::new(&mut file).finish()?;
    let df = df
        .lazy()
        .with_columns(vec![
            col("si").sqrt().alias("y"),
            col("csi").sqrt().alias("x"),
        ])
    ...
    let mut extended_curves = create_extended_curves(curves, 0.2, Some((x_low, x_high)));
    for _ in 0..3 {
        extended_curves.push(predict_next_line(&extended_curves));
    }

    let output_dir = "extended_curves/";
    ...
    for (idx, curve) in extended_curves.iter().enumerate() {
        ...
        let mut file = std::fs::File::create(&format!("extended_curves/curve_{}.parquet", idx))?;
    ...
    let mut df = df
        .lazy()
        .with_column(col("z_lin").alias("z_lin_old"))
        .collect()?;

    let mut df = df
        .with_column(Series::new("z_lin", z_lin))?
        .with_column(Series::new("x_mixed", xs))?;

    let mut file = std::fs::File::create("data/linearized_60.parquet")?;
